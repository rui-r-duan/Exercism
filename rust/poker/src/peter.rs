...
impl<'a> PokerHand<'a> {
    ...
    fn rank(card: &str) -> u32 {
        ...
        let kinds = card
            .split(' ')
            .map(|chars| match &chars[..chars.len() - 1] {
                "J" => 11,
                "Q" => 12,
                "K" => 13,
                "A" => 14,
            ...
            .fold(BTreeMap::<u32, u32>::new(), |mut dict, rank| {
                *dict.entry(rank).or_insert(0) += 1;
                dict
            });
        let mut kind_and_count = kinds.iter().map(|(&x, &y)| (x, y)).collect::<Vec<_>>();
        kind_and_count.sort_by(|a, b| b.1.cmp(&a.1));
        match kind_and_count.as_slice() {
            ...
            _ => {
                let kind_values = kinds.keys().cloned().collect::<Vec<_>>();
                let is_same_suit = suits.windows(2).all(|p| p[0] == p[1]);
                let is_asc_order = kind_values
                    .windows(2)
                    .all(|d| d[0] + 1 == d[1] || d[1] == 14);
...
pub fn winning_hands<'a>(hands: &[&'a str]) -> Vec<&'a str> {
    if hands.len() == 0 {
        ...
        let mut poker_hands = hands.iter().map(|h| PokerHand::new(h)).collect::<Vec<_>>();
        poker_hands.sort_by(|h1, h2| h2.rank.cmp(&h1.rank));
        poker_hands
            .iter()
            .filter(|d| d.rank == poker_hands[0].rank)
            .map(|d| d.card)
            .collect()
    }
}
